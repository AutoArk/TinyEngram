...
class DataArguments:
    data_path: str = field(
        default="dataset/dmrc", metadata={"help": "Path to the processed dataset directory"}
    )
...
def train():
    ...
    # Load dataset from disk (processed parquet format)
    rank0_print(f"Loading files from {data_args.data_path}")
    try:
        ds = load_from_disk(data_args.data_path)
    except Exception as e:
        rank0_print(f"Failed to load_from_disk, trying load_dataset: {e}")
        ds = load_dataset("parquet", data_files=data_args.data_path, split="train")

    rank0_print(f"Loaded dataset size: {len(ds)}")
    
    # Split
    rank0_print("Splitting dataset into train/validation (1% val)...")
